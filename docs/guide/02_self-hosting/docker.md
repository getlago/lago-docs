...
You have 2 possibilities to do it :
...
- Run the script to generates the certificates
...
- Take a look at the `docker-compose.yml` file and uncomment the part for self signed certificate
...
- Edit the file `extra/init-letsencrypt.sh`
  - You should put your domain name by replacing `lago.example`
  - You should put a valid email
...
# This will ask you some informations
# After that you should see a extra/certbot folder
```

- Take a look at the `docker-compose.yml` file and uncomment all the parts for let's encrypt support
