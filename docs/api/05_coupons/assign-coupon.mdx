...
<Tabs>
  ...
  * `unable to find customer`: The customer id does not match an existing customer.
  * `coupon does not exists`: The coupon code does not match an existing coupon.
  * `no_active_subscription`: The customer does not have an active subscription.
  * `coupon_already_applied`: The coupon is already applied to the customer.
