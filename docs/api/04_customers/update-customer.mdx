...
<Tabs groupId="prog-language">
  ...
  ```javascript
  await client.customers.createCustomer({ customer: { externalId: "1234" } });
  ```
  </TabItem>
...
| Attributes | Type | Description |
| -----------| -----| ----------- |
| vat_rate | Float &nbsp &nbsp &nbsp<Optional>**Optional**</Optional> |  Custom VAT applied to the customer.<br/>Will be converted into a [tax rate](../tax_rates/tax-rate-object) object applied the customer to override the organization's one. |

### Metadata

| Attributes | Type | Description |
| -----------| -----| ----------- |
| key | String &nbsp &nbsp &nbsp| Metadata object key. Required only when updating an existing metadata |
...
| id | String &nbsp &nbsp &nbsp<RequiredUnderCondition>**Required conditionally**</RequiredUnderCondition> | Metadata object identifier - only required when updating metadata |
...
<Tabs>
  ...
  **Possible errors:**
  ...
  | `vat_rate` | `multiple_tax_rates` | deprecated `vat_rate` cannot be updated because multiple tax rates are attached to the organization |
