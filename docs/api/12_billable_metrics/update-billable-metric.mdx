...
| aggregation_type | String &nbsp &nbsp &nbsp<Optional>**Optional**</Optional> | Aggregation type that is used in event calulations.<br></br>It can be `count_agg`, `sum_agg`, `max_agg` or `unique_count_agg`.<br></br>It won't be updated if billable metric is attached to any subscriptions |
