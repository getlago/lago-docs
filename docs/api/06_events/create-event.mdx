...
<Tabs groupId="prog-language">
  ...
  let event = new Event({
    ...
    customerId: "__UNIQUE_CUSTOMER_ID__",
...
| **subscription_id** | String &nbsp &nbsp &nbsp<Optional>**Optional**</Optional> | Unique subscription ID in Lago's application.<br></br>`subscription_id` is required if the customer has multiple subscriptions or if `customer_id` is not provided (in case there's only one subscription) |
