...
<Tabs groupId="prog-language">
  ...
  let event = new Event({
    ...
    subscriptionId: "__UNIQUE_SUBSCRIPTION_ID__",
    code: "code",
    timestamp: 1650893379,
  ...
  ```go
    ...
    func main() {
      ...
      eventInput := &lago.EventInput{
        ...
        SubscriptionID: "1dbe81ce-b092-401c-a00b-314292e17a98",
        Code:          "code",
...
| **subscription_id** | String &nbsp &nbsp &nbsp<Optional>**Optional**</Optional> | Unique subscription ID in Lago's application.<br></br>`subscription_id` is required if the customer has multiple subscriptions or if `external_customer_id` is not provided (in case there's only one subscription) |
| **external_customer_id** | String &nbsp &nbsp &nbsp<Optional>**Optional**</Optional> | Customer unique identifier in your application.<br></br>If `subscription_id` is not given, `external_customer_id` is required if there is only one subscription attached to customer. For multiple subscriptions per customer this attribute is not enough |
