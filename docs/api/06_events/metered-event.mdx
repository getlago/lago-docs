...
| **properties** | JSON &nbsp &nbsp &nbsp<Variable>**Variable**</Variable> | Extra data to use for the event aggregation.<br></br>When mandatory, it should contains the `field_name` configured at billable metric level as `key` and any value as field `value`.<br></br><details><summary>**Aggregation type:**</summary><div>- `COUNT`: **not used**<br></br>- `MAX`: **required**. value must be a number<br></br>- `SUM`: **required**. value must be a number<br></br>- `COUNT UNIQUE`: **required**. value could have any datatype<div></div></div></details> |
