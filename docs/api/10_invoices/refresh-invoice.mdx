...
```curl title="PUT"
/api/v1/invoices/:lago_id/refresh
```
...
<Tabs groupId="prog-language">
  ...
  curl --location --request PUT "$LAGO_URL/api/v1/invoices/5eb02857-a71e-4ea2-bcf9-57d3a41bc6ba/refresh" \
  ...
  client.invoices().refresh('5eb02857-a71e-4ea2-bcf9-57d8885990ba') # Invoice ID
  ```
  </TabItem>
  ...
  await client.refreshInvoice('id');
  ```
  </TabItem>
  ...
  ```go
    ...
    func main() {
      ...
      invoice, err := lagoClient.Invoice().Refresh("__YOUR_INVOICE_ID__")
      if err != nil {
        // Error is *lago.Error
        panic(err)
      }
...
<Tabs>
  ...
  The draft invoice has been successfully refreshed.
